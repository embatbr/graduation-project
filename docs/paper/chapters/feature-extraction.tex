...
For each frame the Fast Fourier Transform (FFT) is calculated, with number of points greater than the width of the window (usually 512). Finally, the modulus of the FFT is calculated and the power spectrum is obtained. Due to its symmetry, only the non-negative half is kept.
