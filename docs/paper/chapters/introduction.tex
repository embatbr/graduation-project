...
\chapterref{speaker-recognition-system} contains basic information about voice recognition, as well as the basic architecture for a speaker verification system. The feature extraction process is explained in \chapterref{feature-extraction}, from the reasons for its use to the chosen technique (Mel-Frequency Cepstral Coefficient, MFCC). In \chapterref{gmm} is detailed the GMM and the UBM. \chapterref{fgmm} introduces FCM and the proposed Fractional Gaussian Mixture Model (FGMM). Experiments are described in \chapterref{experiments}, as well as its results. Finally, \chapterref{conclusion} concludes the study. Furthermore, this work contains an appendix with the most relevant pieces of the source code (\appendixref{codes}) and some necessary mathematical concepts.