...
The verification system is constructed using an Universal Background Model-Gaussian Mixture Model (UBM-GMM) to represent all speakers as a single background and a GMM (named Speaker-GMM) or an adaptation of the UBM-GMM (named Adapt-GMM-UBM) throught Bayesian Adaptation, \refbib{Brown, Lee and Spohrer}{brown.lee.spohrer.1983}, both described in details in \chapterref{gmm}. Finally, a likelihood ratio test is used to evaluate a speech signal and decide if it belongs or not to the claimed speaker (see \chapterref{speaker-verification-systems}).
