...
The focus of this paper is on \textbf{text-independent speaker verification} and to achieve that, Gaussian Mixture Models are used in combination with two types of Bayesian Adaptation, \refbib{Brown, Lee and Spohrer}{brown.lee.spohrer.1983}, techniques: Adapt-GMM-UBM and C-Adapt-GMM-UBM. Both are described in details in \chapterref{gmm}.
...
\chapterref{speaker-recognition-system} contains basic information about speaker recognition, as well as the basic architecture of a speaker verification system. The feature extraction process is explained in \chapterref{feature-extraction}, from the reasons for its use to the chosen technique (Mel-Frequency Cepstral Coefficient, MFCC). In \chapterref{gmm} the theory of GMM, Universal Background Model (UBM), Adapt-GMM-UBM and C-Adapt-GMM-UBM are explained. Experiments are described in \chapterref{experiments}, as well as its results. Finally, \chapterref{conclusion} concludes the study. \appendixref{adapt-gmm-ubm-results} shows the results and curves for Adapt-GMM-UBM and \appendixref{c-adapt-gmm-ubm-results} for C-Adapt-GMM-UBM. \appendixref{codes} contains the codes written in Python 3 used to perform the study.