...
The verification system is constructed using an Universal Background Model-Gaussian Mixture Model (UBM-GMM) to represent all speakers as a single background and a GMM (named Speaker-GMM) or an adaptation of the UBM-GMM, throught Bayesian Adaptation, \refbib{Brown, Lee and Spohrer}{brown.lee.spohrer.1983}, for each speaker. Two types of adaptation are performed on the UBM-GMM, Adapt-GMM-UBM and C-Adapt-GMM-UBM, both described in details in \chapterref{gmm}. Finally, a likelihood ratio test is used to evaluate a speech signal and decide if it belongs or not to the claimed speaker (see \chapterref{speaker-verification-systems}).
...
\begin{itemize}\itemsep0pt
    ...
    \item Comparison of Speaker-GMM verification with the verification using Bayesian Adaptation.
    %\item [possible item] Improvement when using FCM
\end{itemize}

\section{Document Structure}

\chapterref{speaker-verification-systems} contains basic information about speaker verification systems, as well as its basic architecture. The feature extraction process is explained in \chapterref{feature-extraction}, from the reasons for its use to the chosen technique (Mel-Frequency Cepstral Coefficient, MFCC). In \chapterref{gmm} the theories of GMM, UBM-GMM, Adapt-GMM-UBM and C-Adapt-GMM-UBM are presented. Experiments are described in \chapterref{experiments}, as well as its results. Finally, \chapterref{conclusion} concludes the study. Furthermore, important contents are presented in the appendices. \appendixref{speaker-gmm-results} shows the results and curves for Speaker-GMM verification, \appendixref{adapt-gmm-ubm-results} for Adapt-GMM-UBM and \appendixref{c-adapt-gmm-ubm-results} for C-Adapt-GMM-UBM.