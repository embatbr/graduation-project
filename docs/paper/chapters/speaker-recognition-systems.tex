...
The idea behind the training stage is to make $\lambda_j$ ``memorize" the distinct characteristics present in the speaker's vocal tract that perform the identification. The GMSM, initially referenced in \sectionref{gmm} and described in details in \chapterref{gmm}, is a perfect choice to model the $\lambda_j$'s.
...
\noindent and the speaker is accepted if $\Lambda(\dvec{X}) \geq \theta$, for an arbitrary value\footnote{$\theta$ is loosely used here. The proper equation would be $\Lambda(\dvec{X}) \geq \log\theta$, but $\theta$ can be used as the system designer prefers, leading to $\Lambda(\dvec{X}) \geq \theta$.} of $\theta$ (see \figureref{speaker-verification}).
