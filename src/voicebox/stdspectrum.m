...
%        (3) x=stdspectrum(5,'t',fs,n) % generate n samples of speech-shaped noise
...
%   5  LTASS-P50    : (obsolete - use 11 instead) the long-term average speech spectrum taken from Table 1 of [4].
...
%  11  LTASS-P50-2  : the long-term average speech spectrum taken from Table 1 of [4].
%                     Converted from mouth reference point @ 0.025m to dB SPL @ 1m on-axis.
%  12  LTASS-1994-2 : better fit to the long-term average speech spectrum that is taken from Table 2 in [2]
%
% References:
...
%      Version: $Id: stdspectrum.m 5624 2015-01-12 12:15:34Z dmb $
%
...
if isempty(spz)
    spty={'White';'A-Weight';'B-Weight';'C-Weight';'LTASS-P50';'LTASS-1994';'SII-IntInv';'BS-468';'USASI';'POTS';'LTASS-P50-2';'LTASS-1994-2'};
    nspty=size(spty,1);
    spz={[1 0];
...
if ~nargout || ~strcmp(m,lower(m))
    ...
    if any(m=='M') || any(m=='Q') || any(m=='E') % draw a frequency response plot
        ...
        pae=any(m=='E') && paz;  % magnitude response error
        ...
        if pae
            ...
            plot([f(1) f(end)],[0 0],':k',f,db(abs(hz))-db(abs(hs)),'-b')
            if pal
                set(gca,'Xscale','log');
            end
            axisenlarge([-1 -1.05]);
